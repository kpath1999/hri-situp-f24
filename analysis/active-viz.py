...
active_path = os.path.join(root, "logs", "analysis", "active_group")
...
for filename in os.listdir(active_path):
    if filename.endswith('.csv'):
        # Read the CSV file
        df = pd.read_csv(os.path.join(active_path, filename))
...
plt.plot(bins[:-1], mean_scores, 'g-', label='Mean Composite Score')
plt.fill_between(bins[:-1], mean_scores - std_scores, mean_scores + std_scores, alpha=0.2, color='g', label='Standard Deviation')
plt.xlabel('Time (minutes)', fontsize=16, fontweight='bold')
plt.ylabel('Composite Score', fontsize=16, fontweight='bold')
plt.title('Active Group - Mean Composite Score and Standard Deviation Over Time', fontsize=20, fontweight='bold')
plt.legend(fontsize=14)
plt.grid(True, linestyle='--', alpha=0.7)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.tight_layout()
plt.savefig('analysis/viz/active_composite_score_over_time.png')
plt.close()
...
print(f"Data saved to 'active_composite_score_over_time.png'")
...
plt.title('Active Group - Mean Posture Metrics Over Time')
plt.legend()
plt.grid(True)
...
plt.tight_layout()
plt.savefig('analysis/viz/active_group_posture_metrics.png')

print("Graph saved as 'active_group_posture_metrics.png'")

# active_path_with_correct_interventions = os.path.join(root, "logs", "archive", "csv")
...
# for filename in os.listdir(active_path_with_correct_interventions):
#     if filename.endswith('.csv'):
#         df = pd.read_csv(os.path.join(active_path_with_correct_interventions, filename))
#         df['timestamp'] = pd.to_datetime(df['timestamp'])
#         df['minutes_elapsed'] = (df['timestamp'] - df['timestamp'].min()).dt.total_seconds() / 60
