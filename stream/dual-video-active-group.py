    ...
    try: 
        while continue_study:
            ...
            if current_time - last_write_time >= 0.5:
                ...
                if math.floor((current_time - start_time)) % 5 == 0:
                ...
                # commented out significant chunks since this is a control study
                if (modality != "none" and bad_posture_duration >= bad_posture_threshold and current_time - last_notification_time >= notification_cooldown):
                    # check to move desk up or down
                    # print(f"lean: {lean_}")
                    if lean_ > 0.4 and (currHeight + timeToMove) <= maxHeight:  # more leaned so move up
                        print("move UP")
                        ser.write(b"up")
                        last_notification_time = current_time
                        currHeight += timeToMove
                        moved_desk = 1
                    elif lean_ <= 0.4 and (currHeight - timeToMove) >= minHeight:
                        print("move DOWN")  # more upright so move down
                        ser.write(b"down")
                        last_notification_time = current_time
                        currHeight -= timeToMove
                        moved_desk = -1
                    ...
                    pass
                    # print(f"bad posture duration: {bad_posture_duration}")
                    # print(f"time from last notif: {current_time - last_notification_time}")
        ...
        # write to a csv file
        csv_output_file = os.path.join(current_dir, '..', 'logs/analysis/active_group', file_name.replace('.pkl', '.csv'))
